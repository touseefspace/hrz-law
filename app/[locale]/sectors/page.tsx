...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tSectors = await getTranslations({ locale, namespace: 'SectorsPage' });
  return {
    title: `${tSectors('title')} | ${tCompany('name')}`,
  };
}

export default async function SectorsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const tSectorsPage = await getTranslations({ locale, namespace: 'SectorsPage' });
             ...
             <div className="lg:w-[320px] shrink-0 top-32">
                <FadeIn className="sticky top-32 editorial-rhythm">
                   ...
                   <ul className="space-y-4 text-sm font-bold tracking-widest uppercase text-foreground/50 border-l rtl:border-l-0 rtl:border-r border-border pl-6 rtl:pl-0 rtl:pr-6">
                      {sectors.map((sector) => (
                         <li key={sector.id} className="hover:text-accent-600 cursor-pointer transition-colors">
                            {tSectors(`${sector.id}.title`)}
                         </li>
                      ))}
                   </ul>
                </FadeIn>
             ...
             <div className="lg:max-w-3xl flex-1">
                <div className="space-y-16">
                  {sectors.map((sector, index) => (
                    <FadeIn key={sector.id} delay={index * 0.05}>
                      <div className="group border-b border-border pb-12 hover:border-accent-500 transition-colors">
                        <span className="text-accent-500 font-bold uppercase tracking-widest text-xs mb-4 block">Sector 0{index + 1}</span>
                        ...
                        <p className="text-foreground/80 text-xl leading-loose">
                           {tSectors(`${sector.id}.description`)}
                        </p>
                      </div>
                    </FadeIn>
                  ))}
